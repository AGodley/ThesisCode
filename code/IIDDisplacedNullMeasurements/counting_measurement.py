...
# Kraus operators are used to update the state, unitary is used to calculate measurements
...
def Trajectory(id, setup):
    ...
    if not Brute:
        ...
        # Just Brute Force
        # brute uses another minimize function after grid method for more accurate results
        bnds = ([(theta - 0.3, theta + 0.3)])  # Interval the argmax investigates
        result, fval, grid, jout = brute(mle, ranges=bnds, Ns=450, full_output=True,
    ...
    if derivative_method:
        ...
        print(f'F from derivative method: {F_dm}')

    # plots MLE function on a grid
    grid = np.linspace(-0.1, 0.5, 200)    # Grid of values
    # axes
    fig = plt.figure()
    ...
    ax.set_title(f'Plot of the MLE with n={n}')
    # Calculates the mle at each point and plots this
    for val in grid:
    ...
    print(
        f'Sample {id+1} finished. It took {np.floor(total_time / 60)} minutes and {total_time % 60} seconds' 
        f'The initial and final estimates of theta were {theta_rough} and {theta_est} respectively'
    )
    ## Sample generation and argmax ##
...
if __name__ == '__main__':
    ...
    if save:
        ...
        with open(Model2.joinpath('counting.csv'), 'a', newline='') as file:
    ...
    if save2:
        # Opens the file in append mode
        with open(Model2.joinpath('counting_thetas.csv'), 'a', newline='') as file:
            data = [F_chains, F_derivative_method, sampleFI, setup['N'], setup['n'], setup['eps'], error, lmbd, phi,
                    setup['Rough'], setup['Sampling'], setup['Derivative method'], setup['Brute']]
            thetas = theta_est
            z = csv.writer(file)
