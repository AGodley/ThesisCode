...
def Trajectory(id, setup):
    ...
    absorber = setup['absorber offset']    # Specified in setup

    # Localized theta
    theta_rough = theta + absorber
    ...
    if True:
        # Finds the Kraus operators at theta_rough=theta
        K_tht0, VU, *_ = kraus(theta, theta+absorber, lmbd, phi, M)
    ...
    if id == 0:
        ...
        K_small_diff, VU, *_ = kraus(theta+h, theta, lmbd, phi, M)
        ...
        # print(K)
        # print(K_tht0)
        # print(K_tht0[0].dag()*K_tht0[0] + K_tht0[1].dag()*K_tht0[1])
        # print(K_dot)
        mu = [None] * order
    ...
    patterns, expected, wsp = pattern_check(x, rho_0_tht0, K, id, absorber)
    ...
    if save3:
        ...
        with open(Model2.joinpath('counting_Markov_excitations_analysis.csv'), 'a', newline='') as file:
            data = [setup['n'], lmbd, phi, setup['absorber offset']] + [val for val in patterns.values()]
            z = csv.writer(file)
    ...
    print(
        '''Sample {} finished. It took {} minutes and {:.1f} seconds. 
        Other statistics: mpn={:.5f}; Traj. avg.={:.5f}; Wsp={:.5f}'''.format(id+1, total_time // 60, total_time % 60, mpn, np.sum(x)/len(x), wsp))
    ## Sample generation and argmax ##
...
def pattern_check(ones_list, stationary_state, ks, trajectory_id, abs_offset):
    ...
    # Total sum of photons from identified patterns weighted by photons in the pattern
    ...
    for key in analysis_x.keys():
        ...
        if trajectory_id == 0:
            ...
            FI_patterns += 4 * (expected_x[key]/len(ones_list)) / (abs_offset)**2

    # Ensures this is only printed once
    if trajectory_id == 0:
        print(f'Analytical result for the m.p.n: {expected_mpn}')
        print(f'Fisher information: {FI_patterns}')

    # Counts the 1s of observed patterns
    weighted_sum_patterns = 0
    # Loop runs over each pattern in the dictionary above
    for key in analysis_x.keys():
        # This converts the string in the dict to the actual pattern found in the trajectory
...
setup = {
    ...
    'n': 100000,  # No. of ancillary systems
    ...
    'absorber offset': 0.01,  # Offset of absorber from true value
}
#==========
## Setup ##
#==========
...
if __name__ == '__main__':
    ...
    # Useful information
    print(f'Running with N={N}, n={n} and absorber offset={setup["absorber offset"]};\n'
          f'')
    ...
    if save:
        # Opens the file in append mode
        with open(Model2.joinpath('counting_Markov_excitations.csv'), 'a', newline='') as file:
    ...
    if save:
        # Opens the file in append mode
        with open(Model2.joinpath('counting_Markov_excitations_thetas.csv'), 'a', newline='') as file:
            data = [sampleFI, setup['N'], setup['n'], setup['eps'], error, lmbd, phi]
            thetas = theta_est
            z = csv.writer(file)
            # Appends data onto the file
    ...
    if save:
        result = results[0]
        # Opens the file in append mode
        with open(Model2.joinpath('counting_Markov_excitations_expected.csv'), 'w', newline='') as file:
            rows = ['n', 'lambda', 'phi', 'absorber offset'] + [key for key in result[5].keys()]
            data = [setup['n'], lmbd, phi, setup['absorber offset']] + [val for val in result[5].values()]
            z = csv.writer(file)
            # Appends data onto the file
            z.writerow(rows)
            z.writerow(data)
