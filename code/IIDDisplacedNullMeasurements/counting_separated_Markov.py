...
from scipy.optimize import minimize, brute, basinhopping     # For argmax/maximum likelihood estimation
...
def Trajectory(id, setup):
    ...
    eps = setup['eps']  # Proportion of trajectory used in initial estimate; not needed
    b_size = setup['batch size']    # No. of samples used in each batch
    ...
    # Calculates an approximate std deviation for the initial est based off the QFI (about 11 per step)
    std_initial = 1 / (sqrt(11*n))

    # Assume localized to 3 times this std deviation
    local = 3*std_initial

    # We apply the absorber at a theta value sufficiently far from the rough estimate
    absorber = theta_rough + 5*std_initial
    ...
    # Splits the remaining samples into batches of size 500
    n_remaining = n_s2
    # Keeps track of how many batches have occurred
    n_batches = 0
    # Variable for averaging the estimates
    avg_theta = 0
    while n_remaining - b_size >= 0:
        # Initializes the state of the s+a
        rho = rho_0
        # List to record which outcomes occurred
        x_batch = [None] * b_size
        for j in np.arange(b_size):
        ...
        # # Brute force mle
        # bnds = ([(theta_rough - local, theta_rough + local)])  # Interval the argmax investigates
        # Ns = 150
        # # print(f'Grid spacing{2*local/Ns}')
        # result, fval, grid, jout = brute(mle_stage2, ranges=bnds, Ns=Ns, full_output=True,
        #                                  args=(theta_rough, lmbd, phi, rho_0, x_rough, x_batch), finish=None)
        # theta_est = result
        theta_est = 0
        ...
        #                     "bounds": bnds,
        #                     "args": (theta_rough, lmbd, phi, rho_0, x_rough, x_batch)}
        # result = basinhopping(mle_stage2, x0=theta_rough, minimizer_kwargs=minimizer_kwargs, niter=3)
        # theta_est = result.x

        # Updates the average variable, batches tracker and remaining samples
        avg_theta += result
        n_batches += 1
        n_remaining -= 500

    # Calculates the average
    avg_theta = avg_theta / n_batches
    ...
    print(
        ...
        The initial and final estimates of theta were {} and {} respectively.
        The grid spacing was {}'''.format(id+1, total_time // 60, total_time % 60, theta_rough, theta_est, 2*local/Ns))
    ## Sample generation and argmax ##
    # End of sample generation
    return F, F_sqrd, F_dm, avg_theta
...
setup = {
    'N': 8,  # No. of samples to generate
    'n': 8000,  # No. of ancillary systems; should be greater than 1000 and a multiple of batch size
    ...
    'batch size':   500     # Size of each batch in trajectory()
}
#==========
## Setup ##
#==========
...
if __name__ == '__main__':
    ...
    print(f'Running with N={N}, n={n} and eps={eps}.\n'
          f'Using {1000} samples in the initial estimation;\n'
          f'And {(n-1000) // b_size} batches of {b_size} samples in the second stage.')
