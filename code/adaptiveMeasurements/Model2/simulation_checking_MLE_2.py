...
theta_est = np.zeros(N)     # Vector of estimates
...
for i in np.arange(len(tht_vals)):
    tht = tht_vals[i]
    ests[i] = mle(tht, lmbd, phi, M_store, rho_0, strat, [measure_system, syst_measurements[0]], opt)
print(ests)

# Argmax for comparison
result = fminbound(mle, bnds[0], bnds[1],
                       args=(lmbd, phi, M_store, rho_0, strat, [measure_system, syst_measurements[0]], opt),
                       xtol=1e-9)
print('Value found by fminbound: {}'.format(result))
