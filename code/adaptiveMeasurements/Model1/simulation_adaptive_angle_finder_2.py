...
# angle_finder_2: Exploring second plane measurements
...
for k in np.arange(len(ang)):
    ...
    M = [np.cos(ang[k]/2) * (fock(2, 0) + np.sin(ang[k]/2) * np.exp(1j*opt)*fock(2, 1)).unit(),
         (np.sin(ang[k]/2) * fock(2, 0) - np.cos(ang[k]/2) * np.exp(1j*opt)*fock(2, 1)).unit()]
...
if save:
    ...
    with open('../../data/Model1/fig_angle_exploration.csv', 'a', newline='') as file:
        z = csv.writer(file)
